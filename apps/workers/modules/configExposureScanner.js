...
export async function runConfigExposureScanner(job) {
    ...
    for (const file of exposedFiles) {
        ...
        for (const [type, info] of secretTypes) {
            await insertFinding(artifactId, 'EXPOSED_SECRETS', `Remove ${file.path} from public access immediately. Move sensitive configuration to environment variables.`, `Found ${info.count} ${type}(s) in ${file.path}`);
        }
    }
    // Summary artifact
